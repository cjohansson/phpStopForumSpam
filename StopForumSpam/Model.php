 ...
 * 3. Verify if a user is classified as spam with the is-functions i.e.: if (!\Models\StopForumSpam\Model::isSpamBotByIpOrEmailOrUsername($ip, $email, $alias) 
 * 
 * @author Christian Johansson <christian@cvj.se>
 * @link http://www.stopforumspam.com
 */

namespace Models\StopForumSpam
{
    ...
    class Model
    {
         ...
         * @return bool
         */
        public static function isSpamBotByIpOrEmailOrUsername($ip, $email, 
         ...
         * @return bool
         */
        private static function _isSpamBot($ip = '', $username = '',
           $email = '', $format = 'json', $debug = false, $operator = 'AND')
        {
            if (!empty($ip)
                || !empty($username)
                || !empty($email)
        ...
        private static function _isSpamEntry($array)
        {
            return (is_array($array)
                && !empty($array['appears'])
                && isset($array['confidence'])
                && $array['confidence'] > self::$spamTolerance);
        }
    ...
    // Debug stuff
    if (\Models\StopForumSpam\Model::$enableDebug) {
        if (!empty($_GET['ip'])
            && !empty($_GET['email'])
        ) {
            if (!empty($_GET['op'])
                ...
                \Models\StopForumSpam\Model::isSpamBotByIpOrEmail(
                    $_GET['ip'], $_GET['email'], true);
            } else {
                \Models\StopForumSpam\Model::isSpamBotByIpAndEmail(
                    $_GET['ip'], $_GET['email'], true);
            }
            ...
            if (!empty($_GET['op'])
                ...
                \Models\StopForumSpam\Model::isSpamBotByIpOrUsername(
                    $_GET['ip'], $_GET['username'], true);
            } else {
                \Models\StopForumSpam\Model::isSpamBotByIpAndUsername(
                    $_GET['ip'], $_GET['username'], true);
            }
        } else if (!empty($_GET['email'])) {
            \Models\StopForumSpam\Model::isSpamBotByEmail(
                $_GET['email'], true);
        } else if (!empty($_GET['ip'])) {
            \Models\StopForumSpam\Model::isSpamBotByIp(
                $_GET['ip'], true);
        } else if (!empty($_GET['username'])) {
            \Models\StopForumSpam\Model::isSpamBotByUsername(
                $_GET['username'], true);
        }
    }
